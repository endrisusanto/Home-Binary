...
export const TerminalLog: React.FC<TerminalLogProps> = ({
  ...
  return (
    <footer 
      className={`fixed inset-x-0 bottom-0 z-40 bg-[#090d16] text-slate-200 border-t border-slate-800 shadow-2xl transition-all duration-200 flex flex-col ${
        !isOpen 
          ? 'h-9' 
          : isMaximized 
            ? 'h-[440px]' 
    ...
    >
      ...
      <div 
        onClick={onToggle}
        className="h-9 px-3.5 bg-[#0f1422] hover:bg-[#141b2e] border-b border-slate-800/80 flex items-center justify-between select-none cursor-pointer transition-colors"
      >
        ...
        <div className="flex items-center gap-2.5 overflow-hidden flex-1 mr-3">
          <div className="flex items-center gap-1.5 font-mono text-xs font-semibold text-slate-200 shrink-0">
            ...
            {logs.length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full text-[9px] font-bold bg-blue-600/30 text-blue-300 border border-blue-500/30">
                {logs.length}
              </span>
            )}
          </div>
          ...
          {!isOpen && latestLog && (
            <div className="hidden sm:flex items-center gap-2 text-[11px] font-mono text-slate-400 truncate opacity-80">
        ...
        <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
          ...
          <button
            onClick={onToggle}
            className="p-1 text-slate-300 hover:text-white hover:bg-slate-800 rounded transition-colors flex items-center gap-1 font-mono text-[11px]"
      ...
      {isOpen && (
        <div 
          ref={scrollRef}
          className="flex-1 p-3 font-mono text-[11px] leading-relaxed overflow-y-auto space-y-1 bg-[#070a12]"
        >
