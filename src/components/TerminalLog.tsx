...
export const TerminalLog: React.FC<TerminalLogProps> = ({
  ...
  return (
    ...
    >
      ...
      <div 
        onClick={onToggle}
        className="h-9 px-3.5 bg-slate-100/90 dark:bg-[#0f1422] hover:bg-slate-200/80 dark:hover:bg-[#141b2e] border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between select-none cursor-pointer transition-colors"
      >
        ...
        <div className="flex items-center gap-2.5 overflow-hidden flex-1 mr-3">
          <div className="flex items-center gap-1.5 font-mono text-xs font-semibold text-slate-800 dark:text-slate-200 shrink-0">
            ...
            {logs.length > 0 && (
              <span className="px-1.5 py-0.2 rounded-full text-[9px] font-bold bg-blue-100 dark:bg-blue-600/30 text-blue-700 dark:text-blue-300 border border-blue-300 dark:border-blue-500/30">
                {logs.length}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
          {isOpen && (
            <>
              ...
              <button
                ...
                className="p-1 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-200/80 dark:hover:bg-slate-800 rounded transition-colors disabled:opacity-30"
                title="Copy logs"
              >
              ...
              <button
                onClick={handleClear}
                disabled={logs.length === 0}
                className="p-1 text-slate-500 hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-400 hover:bg-slate-200/80 dark:hover:bg-slate-800 rounded transition-colors disabled:opacity-30"
                title="Clear logs"
              >
              ...
              <button
                ...
                className="p-1 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-200/80 dark:hover:bg-slate-800 rounded transition-colors"
          ...
          <button
            onClick={onToggle}
            className="p-1 text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white hover:bg-slate-200/80 dark:hover:bg-slate-800 rounded transition-colors flex items-center gap-1 font-mono text-[11px]"
      ...
      {isOpen && (
        <div 
          ref={scrollRef}
          className="flex-1 p-3 font-mono text-[11px] leading-relaxed overflow-y-auto space-y-1 bg-slate-50 dark:bg-[#070a12] text-slate-800 dark:text-slate-200"
        >
          {filteredLogs.length === 0 ? (
            <div className="text-slate-400 dark:text-slate-600 select-none py-8 text-center">
              [No logs captured yet. Execute a batch to stream live output.]
            </div>
          ) : (
            filteredLogs.map((log) => (
              <div key={log.id} className="flex items-start gap-2 hover:bg-slate-200/60 dark:hover:bg-slate-900/60 px-1 py-0.5 rounded transition-colors">
                <span className="text-slate-400 dark:text-slate-500 select-none shrink-0">
                  {log.timestamp}
                </span>
