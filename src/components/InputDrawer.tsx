...
export const InputDrawer: React.FC<InputDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-900/40 dark:bg-black/75 backdrop-blur-xs flex justify-end transition-opacity">
      <div className="w-full max-w-2xl bg-white dark:bg-[#09090b] h-full shadow-2xl border-l border-slate-200 dark:border-neutral-800 flex flex-col animate-in slide-in-from-right duration-200">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4 flex flex-col">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {SAMPLE_TEMPLATES.map((tmpl, idx) => (
              <button
                ...
                className="text-[11px] px-2 py-1 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors font-medium"
              >
                {tmpl.name.split(':')[0]}
              </button>
            ))}
          ...
          <div className="flex-1 flex flex-col space-y-1.5">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center justify-between">
              ...
              <span className="text-[11px] font-normal text-slate-400">
                Columns: Fingerprint, PDA, CSC, Baseband
              </span>
            </label>
            <textarea
              value={rawText}
              ...
              className="w-full flex-1 min-h-[220px] p-3 text-xs font-mono bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 rounded-xl focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:focus:ring-blue-400 outline-none transition-all placeholder:text-slate-400"
            />
          </div>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-2">
              ...
              <div className="space-y-1.5 max-h-36 overflow-y-auto">
                {parsedItems.slice(0, 3).map((item, i) => (
                  <div key={i} className="text-[11px] font-mono bg-white dark:bg-slate-900 p-2 rounded border border-slate-200/60 dark:border-slate-700/60 flex flex-col">
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg shadow-sm shadow-blue-500/20 transition-all"
            >
